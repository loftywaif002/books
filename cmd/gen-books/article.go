...
// data common to Article and Chapter
type Doc struct {
	...
	// extracted from IndexKV, used in book_index.tmpl.html
	Title string
	// base for both filename and url, format: ${ID}-${Title}
	FileNameBase string

	// path of the .md file from which we've read the article
	// for chapter it's 000-index.md file
...
func (a *Article) GitHubURL() string {
	uri := a.Chapter.GitHubURL() + "/" + filepath.Base(a.sourceMdFilePath)
